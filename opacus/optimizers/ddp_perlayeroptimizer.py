...
class DPDDPPerLayerOptimizer(DPOptimizer):
    def __init__(
        self,
        optimizer: Optimizer,
        *,
    ...
    def clip_and_accumulate(self):
        raise NotImplementedError("Clip and accumulate is added per layer in DPDDP Per Layer.")

    def add_noise(self):
        raise NotImplementedError("Noise is added per layer in DPDDP Per Layer.")

    def pre_step(self):
        self.accumulated_iterations = max([p.accumulated_iterations for p in self.params])
        if self.step_hook:
            self.step_hook(self)
    ...
    def _ddp_per_layer_hook(self, p: nn.Parameter, max_grad_norm: float, _: torch.Tensor):
